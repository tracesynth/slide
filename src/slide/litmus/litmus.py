...
from src.slide.litmus.parser.LitmusLexer import LitmusLexer
from src.slide.litmus.parser.LitmusListener import LitmusListener
from src.slide.litmus.parser.LitmusParser import LitmusParser
from src.slide.prog import find_reg_by_name, AmoInst, RFmtInst, IFmtInst, BFmtInst, JFmtInst, StoreInst, \
    LoadInst, JalrInst, parse_program, Label, FenceInst
from src.slide.prog.types import EType, QType
...
class Litmus:
    ...
    def gen_parallel_paths(self):
        from src.slide.analysis import CFG
        from src.slide.analysis.pp import ParallelPath
    ...
    def mutate_new_litmus(self, inject_list, litmus_file_path, final_list = None):
        from src.slide.litmus.litmus_changer import InjectList, InjectType
    ...
    def init_events(self):
        if not self._init_events:
            from src.slide.analysis import Event
            self._init_events = []
            for var in self.vars:
                value = self.get_init_var_value(var)
...
class LitmusParseListener(LitmusListener):
    ...
    def enterFilter(self, ctx: LitmusParser.FilterContext):
        ...
        from src.slide.prog.reg import xregs_abi, xregs_numeric
